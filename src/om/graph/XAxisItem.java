...
{
	// UNED: 28-09-2011 - dballestin - Initialization code moved to initXAxis method
	/**
	...
	{
		float fY=getWorld().convertYFloat(getY());
		...
		boolean bRotate,boolean bFlip,boolean bNumbers,int iExtraOffset)
	{
	  ...
	  iDescent=bNumbers ? 0 : g2.getFontMetrics().getDescent();
		...
		int iCentreX=getWorld().convertX(dPoint),iCentreY=getWorld().convertY(getY());
	...
	protected void paintTick(Graphics2D g2,double dPoint,int iSize,boolean bPos)
	{
		float
		  ...
		  fY=getWorld().convertYFloat(getY());
	...
	// UNED: 28-09-2011 - dballestin
	/**
	 * Set overall axis range.
	 * @param gr Range for axis
	 */
