...
{
	// UNED: 27-09-2011 - dballestin - Initialization code moved to initFunction method
	/**
	...
	public void paint(Graphics2D g2)
	{
		...
		int
			iX1=getWorld().convertX(getRange().getMin()),
			iX2=getWorld().convertX(getRange().getMax());
		...
		for(int iX=iMinX;iX<=iMaxX;iX++)
		{
			...
			float fY=getWorld().convertYFloat(getFunction().f(dX));
			if(iX==iMinX)
				gpPath.moveTo(iX,fY);
			else
				gpPath.lineTo(iX,fY);
	...
	/**
	 * Get range within which to calculate function.
	 * @return Range within which to calculate function
	 */
	...
	/**
	 * Set range within which to calculate function.
	 * @param gr Range within which to calculate function
	 */
	protected void setRange(GraphRange gr)
	{
		this.gr=gr;
	}
}
