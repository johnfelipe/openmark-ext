...
public class QComponentManager
{
	...
	// UNED: 21-06-2011 - dballestin - Changed access modifier of this constructor to protected so
	//                                 it would be possible to declare subclasses in other packages
	// UNED: 21-06-2011 - dballestin - Instead of directly calling om.stdcomponent.ComponentRegistry.fill method 
	//                                 now we call the new defined constructor as this(ip,true) 
	//                                 so the new constructor will know we want that call being done
	/**
	...
	/**
	 * Constructs with standard components plus anything from jar files specified
	 * in InitParams if passed true to registerStandard, otherwise constructs without components.
	 * @param ip Question initialisation parameters
	 * @param registerStandard true to register standard components, false otherwise
	 * @throws OmDeveloperException If the component registry throws a wobbly
	 */
	protected QComponentManager(InitParams ip,boolean registerStandard) throws OmDeveloperException
	{
		if (registerStandard)
		{
