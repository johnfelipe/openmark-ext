...
public abstract class StandardQuestion implements Question
{
	...
	// UNED: 21-06-2011 - dballestin - Now it calls the new init method as 
	//                                 init(d,initParams,new QComponenManager(initParams)) 
	//                                 to preserve old behaviour
	public Rendering init(Document d,InitParams initParams) throws OmException
	{
		return init(d,initParams,new QComponentManager(initParams));
	}
	
	// UNED: 21-06-2011 - dballestin
	/**
	 * Called to initialise new question.<br/><br/>
     * Same as init(Document d,InitParams initParams) but it uses the component manager indicated for managing
     * components.
     * @param d XML document containing question
	 * @param initParams Initialisation parameters for question
	 * @param qcm Component manager
	 * @return Information needed to render question as XHTML
	 * @throws OmException
	 */
	public Rendering init(Document d,InitParams initParams,QComponentManager qcm) throws OmException
	{
		this.ip=initParams;
	...
	// UNED: 21-07-2011 - dballestin - Changed visibility to protected to be able to call
	//                                 this method from subclasses
	/**
	 * Fix placeholders just before returning output.<br/><br/>
	 * @param r Output about to be returned
	 */
	protected void fixPlaceholders(Rendering r)
	{
	...
	// UNED: 21-06-2011 - dballestin
	/**
	 * Protected getter to be able to access component tree from subclasses
	 * @return Component tree
	 */
	protected QDocument getDocument()
	{
		return qd;
	}
	
	// UNED: 21-07-2011 - dballestin
	/**
	 * Protected getter to be able to access current action rendering from subclasses
	 * @return Current action rendering (if we are inside an action() call)
	 */
	protected ActionRendering getCurrentActionRendering()
	{
		return arCurrent;
	}
	...
	// UNED: 21-07-2011 - dballestin
	/**
	 * Protected setter to be able to set current action rendering from subclasses
	 * @param arCurrent Current action rendering (if we are inside an action() call)
	 */
	protected void setCurrentActionRendering(ActionRendering arCurrent)
	{
	...
	// UNED: 21-07-2011 - dballestin
	/**
	 * Protected getter to be able to access map of all currently-set placeholders 
	 * from subclasses
	 * @return Map of all currently-set placeholders
	 */
	...
	/**
	 * Protected getter to be able to access set of checked callbacks
	 * @return Set of checked callbacks
	 */
	protected Set<String> getCheckedCallbacks() {
		return sCheckedCallbacks;
	}
}
