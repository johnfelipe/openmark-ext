...
public class NavigatorServlet extends HttpServlet
{
	...
	// UNED: 11-10-2012 - dballestin - Changed access modifier of this constant to protected so
	//                                 it would be possible to access it from subclasses
	protected static final String COOKIENAME="tnavigator_session";
	// UNED: 11-10-2012 - dballestin - Changed access modifier of this constant to protected so
	//                                 it would be possible to access it from subclasses
	protected static final String ACCESSCOOKIENAME="tnavigator_access";
	// UNED: 11-10-2012 - dballestin - Changed access modifier of this constant to protected so
	//                                 it would be possible to access it from subclasses
	protected static final String FAKEOUCUCOOKIENAME="tnavigator_xid";
	...
	// UNED: 11-10-2012 - dballestin - Changed access modifier of this field to protected so
	//                                 it would be possible to access it from subclasses
	...
	// UNED: 22-02-2012 - dballestin - Changed access modifier of this field to protected so
	//                                 it would be possible to access it from subclasses
	/** Log */
	protected Log l;
	...
	public void init() throws ServletException
	{
		...
		try
		{
			...
			// UNED: 28-09-2012 - dballestin - Now we load configuration from navigation.xml file using
			//								   a protected method so it can be possible to implement new
			//								   configuration options in subclasses.
			//                                 Howewer default implementation in this class will respect
			//								   old configuration options (commented lines).
			//nc=new NavigatorConfig(new File(
			//	sc.getRealPath("navigator.xml")));
			nc=initializeNavigatorConfig(sc);
		}
	...
	// UNED: 22-02-2013 - dballestin - Changed access modifier of this class and its constructor to public so
	//                                 it would be possible to access it from other classes
	@SuppressWarnings("serial")
	public static class StopException extends OmException
	{
		...
		HttpServletRequest request,HttpServletResponse response)
	{
		...
		try
		{
			...
			// UNED: 23-03-2012 - dballestin - Added chance to handle new options/task for Test Navigator
			//                                 at subclasses
			if (handleCustom(bPost,sPath,rt,request,response))
			{
				return;
			}
			...
			synchronized(sessions)
			{
				...
				if(bNewCookie)
				{
					...
					// UNED: 26-11-2012 - dballestin - Now we instantiate user session from 
					//                                 a protected method so it can be possible to do 
					//                                 specific initializatons in subclasses.
					//                                 Howewer default implementation in this class will 
					//                                 respect old behaviour (commented line).
					//us=new UserSession(this, sCookie);
					us=createUserSession(sCookie);
			...
			synchronized(us)
			{
				...
				// UNED: 07-05-2012 - dballestin - Now we do this check calling the new method 
				//								   checkTestLogged
				//if(us.getTestId()==null)
				if (!checkTestLogged(us,sCommand))
				{
					sendError(us,request,response,
				...
				if(!us.isSingle())
				{
					...
					// UNED: 07-05-2012 - dballestin - Now we do checks and handle ?restart command for tests in
					//                                 a protected method so it can be possible to implement a
					//                                 different behaviour in subclasses.
					//                                 Howewer default implementation in this class will respect
					//								   old behaviour (commented lines).
					/*
					...
					*/
					if (sCommand.equals("?restart"))
					{
						if (handleRestartTest(sTestID,rt,bPost,us,request,response))
						{
							return;
						}
					}
	...
	// UNED: 07-05-2012 - dballestin - Changed access modifier of this method to protected so
	//                                 it would be possible to access it from subclasses
	protected void handleStart(RequestTimings rt,String sTestID,UserSession us,int variant,
			HttpServletRequest request, HttpServletResponse response)
		throws Exception
	{
	...
	// UNED: 07-05-2012 - dballestin - Changed access modifier of this method to protected so
	//                                 it would be possible to access it from subclasses
	protected void stopQuestionSession(RequestTimings rt, UserSession us) 
	...
	// UNED: 07-05-2012 - dballestin - Changed access modifier of this method to protected so
	//                                 it would be possible to access it from subclasses
	protected void handleRestart(RequestTimings rt,
		UserSession us,HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		// Start new attempt at question
		...
		UserSession us,HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		if(!us.getTestDefinition().isNavigationAllowed() || bPost)
		{
			...
			// UNED: 22-11-2012 - dballestin - Now we handle sending email for ending a test within
			...
			*/
			if (us.getTestDeployment().requiresSubmitEmail())
			{
				handleEndSubmitEmail(rt,us,us.ud,us.getTestDefinition(),us.getDbTi(),
					us.getTestDefinition().getFinalPage(),us.sOUCU);
			}
...
//				XML.createText(eDiv,"h3","Your answers");
	...
	// UNED: 11-10-2012 - dballestin - Changed access modifier of this method to protected so
	//                                 it would be possible to access it from subclasses
	/**
	 * @param request HTTP request
	 * @return The accessibility cookie, or an empty string if it isn't set
	 */
	protected String getAccessibilityCookie(HttpServletRequest request)
	{
		String sCookie="";
		Cookie[] ac=request.getCookies();
		if(ac==null) ac=new Cookie[0];
			...
			throws StopException
	{
		...
		if(us!=null && title!=ACCESSOUTOFSEQUENCE)
		{
			...
			if (!keepSession) {
				// UNED: 18-02-2013 - dballestin - Now we throw away session using a protected method 
				//                                 so it can be possible to do specific implementations 
				//                                 within subclasses.
				//                                 Howewer default implementation in this class will 
				//                                 respect old behaviour (commented line).
				/*
				...
				*/
				throwAwaySession(request,response,sessions,us);
			}
	...
	private void sendAdminAlertNow()
	{
		try
		{
			synchronized(adminAlertSynch)
			{
				...
				// Send mail now
				// UNED: 21-11-2012 - dballestin - Change to use new protected method sendMail so we have
				//                                 a chance within subclasses to send alert mails with a different 
				//								   implementation than the one provided by om.tnavigator.Mail class
				...
				sendMail(nc.getAlertMailFrom(),null, nc.getAlertMailTo(),nc.getAlertMailCC(),null,
					"Om alert: "+getFriendlyServerName(),adminAlerts.toString(),Mail.TEXTPLAIN);
				adminAlerts=null;
			}
		}
	...
	// UNED: 21-11-2012 - dballestin 
	/**
	 * Sends the given mail message with a given MIME format.<br/><br/>
	 * This implementation uses <i>om.tnavigator.Mail</i> class to send mail.<br/><br/>
	 * Note that at least aCCO addresses are ignored in this implementation.<br/><br/>
	...
	{
		Mail.send(from,replyTo,to,cc,subject,message,mimeType);
	}

	// UNED: 22-02-2012 - dballestin - Changed access modifier of this method to protected so
	//                                 it would be possible to access it from subclasses
	/** @return A 'friendly' version of the server's own name, e.g. pclt166/om-tn */
	protected String getFriendlyServerName()
	{
	 ...
	 */
	protected String displayServletURL(URL u)
	{
	...
	// UNED: 07-05-2012 - dballestin 
	/**
	 * Protected getter to be able to access sessions from subclasses
	 * @return Sessions
	 */
	protected Map<String,UserSession> getSessions()
	{
		return sessions;
	}
	
	// UNED: 28-09-2012 - dballestin
	/**
	 * @param sc Servlet context
	 * @return Initialized navigation configuration from <i>navigation.xml</i> file
	 */
	protected NavigatorConfig initializeNavigatorConfig(ServletContext sc) throws IOException
	{
		return new NavigatorConfig(new File(sc.getRealPath("navigator.xml")));
	}
	
	// UNED: 21-09-2012 - dballestin - New handler for customize new options/tasks for Test Navigator.
	//								   It is supposed to be implemented at subclasses overriding this method
	//								   if needed
	/**
	 * New handler for customize new options/tasks for Test Navigator.<br/><br/>
	 * It is supposed to be implemented at subclasses overriding this method if needed.
	 ...
	 * Default implementation checks if the user has permission to restart it and that the request is POST.
	 * <br/><br/>
	 * However it is declared as protected so it is possible to implement different behaoviour in subclasses.
	 * @param sTestID Test ID
	 * @param rt Request timings
	 ...
	 * @param request Servlet request
	 * @param response Servlet response
	 * @return true if <i>?restart</i> option has been handled (by default if the request is POST), 
	 * false otherwise
	 * @throws Exception
	 */
		...
		HttpServletRequest request,HttpServletResponse response) throws Exception
	{
		...
		{
			if(!us.getTestDefinition().isRedoTestAllowed() && !us.bAdmin)
				sendError(us,request,response,HttpServletResponse.SC_FORBIDDEN,true,false,null,"Forbidden",
					"This test does not permit restarting it.",null);
			else
				handleRestart(rt,us,request, response);
		}
		return bPost;
	...
	// UNED: 07-05-2012 - dballestin 
	/**
	 * Check if we are logged into a test.
	 * @param us User session
	 * @param sCommand Command string
	 * @return true if we are logged into a test, false otherwise
	 */
	protected boolean checkTestLogged(UserSession us,String sCommand)
	{
		return us.getTestId()!=null;
	}
	
	//UNED: 22-11-2012 - dballestin
	/**
	 * Handles sending emails after an user ends a test.<br/><br/>
	 * Default implementation sends a confirmation email if desired (to non-admins and only people 
	 * who look like students).<br/><br/>
	...
	{
		if (!us.bAdmin && ud.shouldReceiveTestMail())
		{
			String sEmail=IO.loadString(
				new FileInputStream(getServletContext().getRealPath("WEB-INF/templates/submit.email.txt")));
			Map<String,String> mReplace=new HashMap<String,String>();
			mReplace.put("NAME",td.getName());
			mReplace.put("TIME",(new SimpleDateFormat("dd MMMM yyyy HH:mm")).format(new Date()));
			sEmail=XML.replaceTokens(sEmail,"%%",mReplace);
			try
			{
				String token=auth.sendMail(sOUCU,ud.getPersonID(),sEmail,Authentication.EMAIL_CONFIRMSUBMIT);
				StringBuffer logEmailSent=new StringBuffer("Sent submit confirm email to ");
				logEmailSent.append(sOUCU);
				logEmailSent.append(" for ");
				logEmailSent.append(us.getTestId());
				logEmailSent.append(" (message despatch ID ");
				logEmailSent.append(token);
				logEmailSent.append(')');
			...
			{
				StringBuffer logEmailSentError=new StringBuffer("Error sending submit confirm email to ");
				logEmailSentError.append(sOUCU);
				logEmailSentError.append(" for ");
				logEmailSentError.append(us.getTestId());
				getLog().logError(logEmailSentError.toString(),e);
				us.setEmailSent(-1);
			}
		}
	}
	
	//UNED: 26-11-2012 - dballestin
	/**
	 * Instantiate a new user session.<br/><br/>
	 * You can override this method within subclasses if you need a different behaviour.
	 * @param sCookie User session cookie
	 * @return New user session
	 */
	protected UserSession createUserSession(String sCookie)
	{
		return new UserSession(this,sCookie);
	}
	
	//UNED: 18-02-2013 - dballestin
	/**
	 * Throw away session.<br/><br/>
	 * This method is invoked on error from <i>sendError</i> method if <i>keepSession</i> is false.<br/><br/>
	 * Default implementation throws away user session on this test but keeps user logged in.<br/><br/>
	 * You can override this method within subclasses if you need a different behaviour.
	 * @param request HTTP request
	 * @param response HTTP response
