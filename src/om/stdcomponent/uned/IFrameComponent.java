 ...
 * </table>
 */
public class IFrameComponent extends om.stdcomponent.IFrameComponent implements Testable
{
	...
	{
		PROPERTY_DISPLAY,PROPERTY_ENABLED,PROPERTY_LANG,PROPERTY_ACTION,PROPERTY_WIDTH,PROPERTY_HEIGHT,
		PROPERTY_CUSTOM,PROPERTY_TEST,PROPERTY_ANSWER,PROPERTY_ATTEMPTSMIN,PROPERTY_ATTEMPTSMAX,
	 ...
	 * window, inialize 'external' field that means the page is not in the question jar and if the page 
	 * is not external check for 'action' property for checking that a callback method is present and correctly 
	...
	protected void initializeSpecific(Element eThis) throws OmException
	{
		...
		// We need to initialize 'external' field that means the page is not in the question jar, 
		// we want to keep this definition because internal pages can send data back to OM, externals cant 
		// - SECURITY!!
		external=getString(PROPERTY_SRC).substring(0,4).equalsIgnoreCase("http");
		
		// If the page is not external we add a check for 'action' property for checking that a callback method 
		// is present and correctly defined for its value
		if (!external)
		{
			addSpecificCheck(
					PROPERTY_ACTION,
	...
	/**
	 * Tests if this iframe component match all conditions to be displayed.
	 * @param idAnswer Identifier of component selected for answer
	 * @return true if this iframe component match all conditions to be displayed, false otherwise
	 */
	@Override
