...
/**
 * A layout that simply arranges child components in a random order.<br/><br/>
 ...
 * <tr><td>shuffle</td><td>(boolean)</td><td>If true, randomises order of children. By default is true.
 * </td></tr>
 * <tr><td>fixedpositions</td><td>(string)</td><td>Comma-separated list of fixed positions not affected by
 * shuffle property.</td></tr>
 * <tr><td>randomgroup</td><td>(string)</td><td>Identifier of the &lt;random&gt; component or name of group 
 * used as seed for random number generator or null if we are using default seed</td></tr>
 ...
 * </table>
 */
public class LayoutShuffleComponent extends QComponent implements Testable
{
	/** Boolean property: whether or not to shuffle the contents */
	public final static String PROPERTY_SHUFFLE="shuffle";
	
	/** 
	 * Property name with a comma-separated list of fixed positions not affected by shuffle property 
	 * (string).
	 */
	public static final String PROPERTY_FIXEDPOSITIONS="fixedpositions";
	
	/**
	 * Property name with the identifier of the &lt;random&gt; component or the name of group used as seed 
	 * for random number generator or null if we are using default seed (string).
	 */
	public static final String PROPERTY_RANDOMGROUP="randomgroup";
	...
	{
		PROPERTY_DISPLAY,PROPERTY_ENABLED,PROPERTY_LANG,PROPERTY_SHUFFLE,PROPERTY_FIXEDPOSITIONS,
		PROPERTY_RANDOMGROUP,PROPERTY_TEST,PROPERTY_ANSWER,PROPERTY_ATTEMPTSMIN,PROPERTY_ATTEMPTSMAX,
		PROPERTY_SELECTEDANSWERSMIN,PROPERTY_SELECTEDANSWERSMAX,PROPERTY_SELECTEDRIGHTANSWERSMIN,
		PROPERTY_SELECTEDRIGHTANSWERSMAX,PROPERTY_SELECTEDWRONGANSWERSMIN,PROPERTY_SELECTEDWRONGANSWERSMAX,
		PROPERTY_UNSELECTEDANSWERSMIN,PROPERTY_UNSELECTEDANSWERSMAX,PROPERTY_UNSELECTEDRIGHTANSWERSMIN,
		PROPERTY_UNSELECTEDRIGHTANSWERSMAX,PROPERTY_UNSELECTEDWRONGANSWERSMIN,PROPERTY_UNSELECTEDWRONGANSWERSMAX,
		PROPERTY_RIGHTDISTANCEMIN,PROPERTY_RIGHTDISTANCEMAX,PROPERTY_ANSWERTYPE,PROPERTY_CASESENSITIVE,
		PROPERTY_TRIM,PROPERTY_STRIP,PROPERTY_SINGLESPACES,PROPERTY_NEWLINESPACE,PROPERTY_IGNORE,
		PROPERTY_IGNOREREGEXP,PROPERTY_IGNOREEMPTYLINES,PROPERTY_TOLERANCE
	};
	...
	protected void defineProperties() throws OmDeveloperException
	{
		...
		setBoolean(PROPERTY_SHUFFLE,true);
		
		defineString(PROPERTY_FIXEDPOSITIONS);
		setString(PROPERTY_FIXEDPOSITIONS,null);
		
		defineString(PROPERTY_RANDOMGROUP);
		setString(PROPERTY_RANDOMGROUP,null);
	...
	/** @return true if child components will be displayed in a random order, false otherwise */
	public boolean isShuffle()
	{
		try
		{
			return getBoolean(PROPERTY_SHUFFLE);
		}
	...
	/**
	 * Set the flag that indicates that child components will be displayed in a random order (true) 
	 * or not (false).
	 * @param shuffle true to indicate that child components will be displayed in a random order, 
	 * false otherwise
	 */
	public void setShuffle(boolean shuffle)
	{
		try
		{
			setBoolean(PROPERTY_SHUFFLE,shuffle);
		}
		catch (OmDeveloperException e)
		{
			throw new OmUnexpectedException(e);
	...
	/**
	 * @return Comma-separated list of fixed positions not affected by shuffle property
	 */
	public String getFixedPositions()
	{
		try
		{
			String fixedPositions=getString(PROPERTY_FIXEDPOSITIONS);
			if (placeholders.containsKey(PROPERTY_FIXEDPOSITIONS))
			{
				checkFixedPositions(fixedPositions);
			}
	...
	/**
	 * Set the comma-separated list of fixed positions not affected by shuffle property.
	...
	public void setFixedPositions(String fixedPositions)
	{
		...
		{
			checkFixedPositions(fixedPositions);
			setString(PROPERTY_FIXEDPOSITIONS,fixedPositions);
		}
		catch (OmDeveloperException e)
		{
			throw new OmUnexpectedException(e);
	 ...
	 */
	private void checkFixedPositions(String fixedPositions) throws OmFormatException
	{
		if (fixedPositions!=null)
		{
			...
			{
				String error="<layoutshuffle>: property 'fixedpositions' has an invalid value";
				try
				{
					int pos=Integer.parseInt(position);
					if (pos<0)
					{
						throw new OmFormatException(error);
					}
				}
	 ...
	 * for random number generator or null if we are using default seed
	 */
	public String getRandomGroup()
	{
		try
		{
			return getString(PROPERTY_RANDOMGROUP);
		}
		catch (OmDeveloperException e)
		{
			throw new OmUnexpectedException(e);
	 ...
	 * random number generator or null to use default seed.
	 * @param randomGroup Identifier of the &lt;random&gt; component or name of group to use as seed 
	 * for random number generator or null to use default seed
	 */
	public void setRandomGroup(String randomGroup)
	{
		try
		{
			setString(PROPERTY_RANDOMGROUP,randomGroup);
		}
		catch (OmDeveloperException e)
		{
			throw new OmUnexpectedException(e);
	 ...
	 * placeholders.<br/><br/>
	 * Default implementation of this method add a check for 'fixedpositions' property.<br/><br/>
	...
	{
		// We add a check for 'fixedpositions' property 
		// for its value
		addSpecificCheck(
				PROPERTY_FIXEDPOSITIONS,
				new PropertyCheck()
				{
					@Override
					public void check(Object value) throws OmDeveloperException
					{
						checkFixedPositions((String)value);
					}
				});
	}
	 ...
	 * It is recommended that you invoke this method from initializeSpecific method once for every property with
	 * specific checks.
	...
	public void produceVisibleOutput(QContent qc,boolean bInit,boolean bPlain) throws OmException
	{
		...
		if (getShuffleMap()==null || getShuffleMap().length!=aqcChildren.length)
		{
			generateSuffleMap(aqcChildren,bShuffle);
		}

		int iChild;
		for(iChild=0;iChild<aqcChildren.length;iChild++)
		{
			aqcChildren[getShuffleMap()[iChild]].produceOutput(qc,bInit,bPlain);
		}
	}
	...
	/**
	 * Set shuffle map
	 * @param shuffleMap Shuffle map
	 */
	protected void setShuffleMap(int[] shuffleMap)
	{
		aiShuffleMap=shuffleMap;
	}
	
	/**
	 * Initialize shuffle map and shuffles it if desired
	 * @param aqcChildren Array of children components
	 * @param bShuffle true to shuffle map, false otherwise
	 */
	protected void generateSuffleMap(QComponent[] aqcChildren,boolean bShuffle)
	{
		String fixedPositions=getFixedPositions();
		if (!bShuffle || fixedPositions==null)
		{
			// Init default shuffle map (unshuffled)
			setShuffleMap(new int[aqcChildren.length]);
			for (int i=0;i<getShuffleMap().length;i++)
			{
				getShuffleMap()[i]=i;
			}
			...
			if (bShuffle)
			{
				...
				// Note that this is true only if they share the same random group
				Random random=getRandomGroup()==null?
					getQuestion().getRandom():getQuestion().getRandom(getRandomGroup(getRandomGroup()));
				Shuffler.shuffle(getShuffleMap(),random);
			}
		}
		else
		{
			// First we get a list with fixed positions as integers
			List<Integer> fixedPosList=new ArrayList<Integer>();
			String[] fixedPositionsArr=fixedPositions.split(",");
			for (String fixedPosition:fixedPositionsArr)
			{
				int pos=Integer.parseInt(fixedPosition);
				if (pos<aqcChildren.length && !fixedPosList.contains(Integer.valueOf(pos)))
				{
					fixedPosList.add(new Integer(pos));
				}
			}
			
			// Init a shuffle map without fixed positions (unshuffled)
			int[] notFixedShuffleMap=new int[aqcChildren.length-fixedPosList.size()];
			int iFixed=0;
			for (int i=0;i<aqcChildren.length;i++)
			{
				if (!fixedPosList.contains(Integer.valueOf(i)))
				{
					notFixedShuffleMap[iFixed]=i;
					iFixed++;
				}
			}
			
			// Shuffle map without fixed positions
			Random random=getRandomGroup()==null?
				getQuestion().getRandom():getQuestion().getRandom(getRandomGroup(getRandomGroup()));
			Shuffler.shuffle(notFixedShuffleMap,random);
			
			// Get a shuffle map with all positions
			int[] shuffleMap=new int[aqcChildren.length];
			int iNotFixed=0;
			for (int i=0;i<aqcChildren.length;i++)
			{
				if (fixedPosList.contains(Integer.valueOf(i)))
				...
				{
					shuffleMap[i]=notFixedShuffleMap[iNotFixed];
					iNotFixed++;
				}
			}
			
			// Set generated shuffle map as layoutshuffle's shuffle map
	...
	/**
	 * Tests if this &lt;layoutshuffle&gt; component match all conditions to be displayed.
	 * @param idAnswer Identifier of component selected for answer
	 * @return true if this &lt;layoutshuffle&gt; component match all conditions to be displayed, 
	 * false otherwise
	 */
	@Override
