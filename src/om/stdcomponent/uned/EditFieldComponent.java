...
import om.stdcomponent.EquationComponent;
import om.stdcomponent.LabelComponent;
...
// UNED: 29-06-2011 - dballestin
/**
 * Standard XHTML edit field for entering a single line or multiple lines of plain text. Can estimate
 * its own size for inclusion within {@link EquationComponent}s.  Default is one line.<br/><br/>
 * <h2>XML usage</h2>
 * &lt;editfield/&gt;
 * <h2>Properties</h2>
 * <table border="1">
 ...
 * <tr><td>lang</td><td>(string)</td><td>Specifies the language of the content, like the HTML lang 
 * attribute. For example 'en' = English, 'el' - Greek, ...</td></tr>
 ...
 * <tr><td>newlinespace</td><td>(boolean)</td><td>Specifies if new line characters from answer are 
 ...
 * <tr><td>ignoreemptylines</td><td>(boolean)</td><td>Specifies if empty lines from answer are 
 ...
 * when using "displayright" task with an editfield with an answertype different from "text", 
 ...
 */
public class EditFieldComponent extends om.stdcomponent.EditFieldComponent implements Answerable
{
	/** Text answer's type. It simply compare the text entered by user with the right attribute */
	...
	/** 
	 * Property name for considering new line characters as whitespaces when testing if it is right 
	 * (boolean)<br/><br/>Note that trim, strip and singlespaces properties are affected if this 
	 ...
	 * with an editfield with an answertype different from "text".<br/><br/>
	...
	{
		PROPERTY_DISPLAY,PROPERTY_ENABLED,PROPERTY_LANG,PROPERTY_COLS,PROPERTY_ROWS,PROPERTY_LABEL,
		PROPERTY_FORCEWIDTH,PROPERTY_FORCEHEIGHT,PROPERTY_ANSWERENABLED,PROPERTY_RIGHT,PROPERTY_ANSWERTYPE,
	...
	/** Temporal value to use inside produceVisibleOutput method */
	...
	/** 
	 * @return true if new line characters from answer are considered as whitespaces when testing if 
	...
	/**
	 * Sets if new line characters from answer are considered to be whitespaces when testing if it is 
	 * right (true) or not (false).<br/><br/>
	 * Note that trim, strip and singlespaces properties are affected if this property is set to yes.
	 * @param newlinespace true if if new line characters from answer are considered as whitespaces, 
	 * false otherwise
	 */
	...
	/**
	 * @return true if empty lines from answer are ignored when testing if it is right, false otherwise
	 */
	...
	/**
	 * Sets if empty lines from answer are ignored when testing if it is right (true) or not (false).
	 * @param ignoreemptylines true if if empty lines from answer are ignored, false otherwise
	 */
	...
	/** 
	 * @return Exact answer to be used for displaying when using "displayright" task with an editfield 
	...
	/**
	 * Sets the exact answer to be used for displaying when using "displayright" task with an editfield 
	 * with an answertype different from "text".
	 * @param displayableRight Exact answer to be used for displaying when using "displayright" task 
	 * with an editfield with an answertype different from "text"
	 */
	...
	private String getPreprocessedAnswer(AnswerTestProperties overrideProperties)
	{
		...
		{
			answer=AnswerChecking.stripWhitespace(
					answer,overrideProperties.getNewLineSpace(isNewLineSpace()));
		}
		else
		{
			if (overrideProperties.getTrim(isTrim()))
			{
				answer=AnswerChecking.trimWhitespace(
						answer,overrideProperties.getNewLineSpace(isNewLineSpace()));
			}
			if (overrideProperties.getSingleSpaces(isSingleSpaces()))
			{
				answer=AnswerChecking.singledWhitespace(
						answer,overrideProperties.getNewLineSpace(isNewLineSpace()));
			}
		}
		if (overrideProperties.getIgnoreEmptyLines(isIgnoreEmptyLines()))
		{
			answer=AnswerChecking.removeEmptyLines(answer);
		}
	...
	/**
	 * Returns if the answer typed in this editfield corresponds to the answer received 
	 * for answers of type text.
	 * @param s String with the expected answer
	 * @param overrideProperties Utility class instance used to allow other classes to override some
	 * properties of this class for testing answer purposes
	 * @return true if the answer typed in this editfield corresponds to the answer received,
	...
	/**
	 * Returns if the answer typed in this editfield corresponds to the numeric value indicated.
	 * @param answer String with the user's answer
	 * @param s String with the expected number
	 * @param options Numeric options
	 * @param overrideProperties Utility class instance used to allow other classes to override some
	 * properties of this class for testing answer purposes
	 * @return true if the answer typed in this editfield corresponds to the numeric value indicated,
	...
	public boolean isAnswer(String s,AnswerTestProperties overrideProperties)
	{
		...
		if (s!=null)
		{
			...
			if (answerType!=null)
			{
				...
				{
					answer=isAnswerText(s,overrideProperties);
				}
				...
				else if (onlyAnswerType.equals(ANSWERTYPE_PMATCH))
				{
					...
					answer=isAnswerPMatch(s,selector,overrideProperties);
				}
				else if (onlyAnswerType.equals(ANSWERTYPE_NUMERIC))
				{
					String selector=getAnswerTypeSelector(answerType);
					answer=isAnswerNumeric(s,selector,overrideProperties);
				}
				else if (onlyAnswerType.equals(ANSWERTYPE_NUMPMATCH))
				{
					String selector=getAnswerTypeSelector(answerType);
					answer=isAnswerNumPMatch(s,selector,overrideProperties);
				}
			}
		}
		return answer;
	...
	{
		return getPreprocessedAnswer(new AnswerTestProperties()).replace('\r', ' ').replace('\n', ' ');
	}
	
	/**
	 * Returns if the answer to display when using "displayright" task with an editfield with a "regexp" 
	 * answertype is right (true) or not (false).
	 * @return true if the answer to display when using "displayright" task with an editfield with a "regexp" 
	 * answertype is right, false otherwise (for answers of type regexp)
	 */
	...
	/**
	 * Returns if the answer to display when using "displayright" task with an editfield with a "pmatch" 
	 * answertype is right (true) or not (false).
	 * @param options PMatch options
	 * @return true if the answer to display when using "displayright" task with an editfield 
	...
	/**
	 * Returns if the answer to display when using "displayright" task with an editfield with a "numeric" 
	 * answertype is right (true) or not (false).
	 * @param options Numeric options as string
	 * @return true if the answer to display when using "displayright" task with an editfield 
	...
	/**
	 * Returns if the answer to display when using "displayright" task with an editfield with a "numpmatch" 
	 * answertype is right (true) or not (false).
	 * @param options Number separator and PMatch options
	 * @return true if the answer to display when using "displayright" task with an editfield 
	...
	/**
	 * Returns if the answer to display when using "displayright" task with an editfield 
	...
	private String getPreprocessedDisplayableRight()
	{
		...
		if (isStrip())
		{
			displayableRight=AnswerChecking.stripWhitespace(displayableRight,isNewLineSpace());
		}
		else
		{
			if (isTrim())
			{
				displayableRight=AnswerChecking.trimWhitespace(displayableRight,isNewLineSpace());
			}
			if (isSingleSpaces())
			{
				displayableRight=AnswerChecking.singledWhitespace(displayableRight,isNewLineSpace());
			}
		}
		if (isIgnoreEmptyLines())
		{
			displayableRight=AnswerChecking.removeEmptyLines(displayableRight);
		}
		if (getIgnore()!=null)
		{
