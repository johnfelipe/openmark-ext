...
/**
 * This is a component used to display text for several answers.<br/>
 * It can be declared everywhere inside question.xml and can contain other content.<br/>
 * Contained text and components define the template to generate the data to display.<br/>
 * The data displayed is generated repeating the template for every answer passed.<br/>
 * Particularly &lt;t&gt; and &ltattribute;&gt; components make their work with the value associated 
 * to each answer in every iteration.<br/>
 * <h2>XML usage</h2>
 * &lt;for&gt;...&lt;/for&gt;
 * <h2>Properties</h2>
 * <table border="1">
 ...
 * <tr><td>type</td><td>(string)</td><td>Iteration type: allIdAnswers, idAnswers (default)</td></tr>
 * <tr><td>separator</td><td>(string)</td><td>Specifies the separator string between iterations, 
 * by default ", "</td></tr>
 * </table>
 */
public class ForComponent extends QComponent
{
	/** Type for iterate all answer components (even not used) */
	public static final String TYPE_ALL_ID_ANSWERS="allidanswers";
	
	/** Type for iterate received answer components (default) */
	public static final String TYPE_ID_ANSWERS="idanswers";
	
	/** Property name for the separator string between iterations, by default ", " (int) */
	public static final String PROPERTY_SEPARATOR="separator";
	
	/** Property name for iteration type: allIdAnswers, idAnswers (default) */
	public static final String PROPERTY_TYPE="type";
	
	/** Default separator string between iterations */
	...
	{
		PROPERTY_DISPLAY,PROPERTY_ENABLED,PROPERTY_LANG,PROPERTY_TYPE
	};
	
	/** Regular expression restricting advancedfield types */
	private static final String PROPERTYRESTRICTION_TYPE="allidanswers|idanswers";
	...
	protected void defineProperties() throws OmDeveloperException {
		...
		defineString(PROPERTY_SEPARATOR);
		setString(PROPERTY_SEPARATOR,DEFAULT_SEPARATOR);
		
		defineString(PROPERTY_TYPE,PROPERTYRESTRICTION_TYPE);
		setString(PROPERTY_TYPE,TYPE_ID_ANSWERS);
	}
	
	/** @return Separator string between iterations. */
	public String getSeparator()
	{
		try
		{
			return getString(PROPERTY_SEPARATOR);
		}
	...
	/**
	 * Sets the separator string between iterations.
	 * @param separator Separator string between iterations
	 */
	public void setSeparator(String separator)
	{
		try
		{
			setString(PROPERTY_SEPARATOR,separator);
		}
		catch(OmDeveloperException e)
		{
			throw new OmUnexpectedException(e);
	...
	/**
	 * Set iteration type: allIdAnswers, idAnswers
	...
	public void produceVisibleOutput(QContent qc,boolean bInit,boolean bPlain) throws OmException
	{
		...
		if (itAnswers!=null)
		{
			String replacedSeparator=getSeparator();
			if (sq.isPlaceholdersInitialized() && StandardQuestion.containsPlaceholder(replacedSeparator))
			{
				replacedSeparator=sq.applyPlaceholders(replacedSeparator);
			}
			for (String idAnswer:itAnswers)
			{
				if (insertSeparator)
				{
					qc.addInlineXHTML(qc.getOutputDocument().createTextNode(getSeparator()));
					qc.addTextEquivalent(replacedSeparator);
				}
				else
				{
					insertSeparator=true;
				}
				sq.pushForIdAnswer(idAnswer);
