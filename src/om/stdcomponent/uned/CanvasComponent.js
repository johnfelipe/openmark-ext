...
// UNED: 21-10-2011 - dballestin - Added markerNum as a parameter instead of calculate it from number
//                                 of markers because now a marker's number not necessarily match 
//                                 its marker's index in the markers array of a canvas
function canvasMarkerInit(canvasId,idPrefix,labelJS,originX,originY,factorX,factorY,markerNum)
{
  // Get marker, canvas, and x/y fields and init marker object
  var canvas=getCanvas(canvasId,idPrefix);
  
  // UNED: 21-10-2011 - dballestin - Comented because markerNum is now received as an argument
  //var markerNum=canvas.markers.length;
  
  var marker=document.getElementById(idPrefix+canvasId+'_marker'+markerNum);
  
  // UNED: 24-10-2011 - dballestin - Added new field markerNum the the marker
...
// UNED: 24-10-2011 - dballestin - Added new function canvasGetMarker to be able to get a marker 
//                                 of a canvas by its markerNum
function canvasGetMarker(canvas,markerNum)
{
  var m=null;
  for (var index=0;m==null && index<canvas.markers.length;index++)
  {
	if (canvas.markers[index].markerNum==markerNum)
	{
      m=canvas.markers[index];
	}
  }
  return m;
}
...
// UNED: 24-10-2011 - dballestin - Changed the implementation to get markers using the new function
//                                 canvasGetMarker(canvas,markerNum) because now a marker's number 
//                                 not necessarily match its marker's index in the 
//                                 markers array of a canvas
function canvasLineInit(canvasId,idPrefix,from,to,labelJS,originX,originY,factorX,factorY)
{
  ...
  // UNED: 24-10-2011 - dballestin - We substitute these lines by calls to the new function 
  //                                 canvasGetMarker(canvas,markerNum)
  //line.from=line.canvas.markers[from];
  //line.to=line.canvas.markers[to];
  line.from=canvasGetMarker(line.canvas,from);
  line.to=canvasGetMarker(line.canvas,to);
