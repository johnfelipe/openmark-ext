...
import om.helper.uned.ImageUtils;
...
// UNED: 17-08-2011 - dballestin
/**
 * Represents a displayed image from a file.<br/><br/>
 * Note that has been added support for bmp and wbmp file formats and also .jpeg extension is now accepted as an 
 * image/jpeg image.<br/><br/>
 ...
 */
public class ImageComponent extends om.stdcomponent.ImageComponent implements Testable
{
	/**
	...
	/** Property name for image's width */
	public final static String PROPERTY_WIDTH="width";
	
	/** Property name for image's height */
	...
	{
		PROPERTY_DISPLAY,PROPERTY_ENABLED,PROPERTY_LANG,PROPERTY_ALT,PROPERTY_FILEPATH,PROPERTY_WIDTH,
		PROPERTY_HEIGHT,PROPERTY_TEST,PROPERTY_ANSWER,PROPERTY_ATTEMPTSMIN,PROPERTY_ATTEMPTSMAX,
	...
	/**
	 * Tests if this image component match all conditions to be displayed.
	 * @param idAnswer Identifier of component selected for answer
	 * @return true if this text component match all conditions to be displayed, false otherwise
	 */
	@Override
	...
	@Override
	protected String getImageMIME(String sFilePath) throws OmException
	{
		int iExt=sFilePath.lastIndexOf('.');
		String ext="";
		if (iExt!=-1 && iExt<sFilePath.length()-1)
		{
			ext=sFilePath.substring(iExt+1);
		}
		String sMimeType=MIMES.get(ext.toLowerCase());
		if (sMimeType==null)
		{
			StringBuffer error=new StringBuffer("Invalid image file type: ");
			error.append(sFilePath);
	...
	{
		if (isPropertySet(PROPERTY_WIDTH) && isPropertySet(PROPERTY_HEIGHT))
		{
			setImageWidth(getInteger(PROPERTY_WIDTH));
			setImageHeight(getInteger(PROPERTY_HEIGHT));
		}
		else
		{
			try
			{
				int[] imageDimensions=ImageUtils.getImageDimensions(getImageData(),getMimeType());
				setImageWidth(imageDimensions[0]);
				setImageHeight(imageDimensions[1]);
			}
			catch (Exception e)
			{
				StringBuffer error=new StringBuffer("Can't read image size from file ");
				error.append(getFilePath());
				throw new OmException(error.toString(),e);
			}
		}
	}
}
